...
// appear on a real map (`MAP_NORTHERN_NORWAY`; `MAP_STANDARD` is still
// `.`/`#` only). `sea` and `air` have no unit yet (ROADMAP.md phase 3), so
// their rows are inert placeholders, not real decisions:
